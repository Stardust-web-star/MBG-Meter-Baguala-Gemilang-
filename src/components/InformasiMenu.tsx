...
export function InformasiMenu({ 
  ...
  const metrics = useMemo(() => {
    ...
    // Energy recovery calculations (standard PLN PLN RP benchmark: 185 kWh/meter x Rp 1.444,70 / kWh)
    const activeGangguan = meterGangguan > 0 ? meterGangguan : 64;
    const estimatedKwh = Math.round(activeGangguan * 185);
    ...
    const tarifChartData = standardTarifs.map(t => ({
      name: t,
      value: tarifCountMap[t] > 0 ? tarifCountMap[t] : (t === 'R1MT' ? 155 : t === 'R1T' ? 98 : t === 'R1' ? 45 : t === 'R1M' ? 25 : 1)
    }));

    // Daily Completion Speed Trend Data
    const dailyTrendData = [
      { date: '08-21', selesai: 15, pending: 1 },
      { date: '08-05', selesai: 17, pending: 1 },
      { date: '08-07', selesai: 16, pending: 1 },
      { date: '08-11', selesai: 15, pending: 2 },
      { date: '08-19', selesai: 14, pending: 3 },
      { date: '08-04', selesai: 15, pending: 0 },
      { date: '08-06', selesai: 14, pending: 0 },
      { date: '08-10', selesai: 16, pending: 1 },
      { date: '08-12', selesai: 15, pending: 2 },
      { date: '08-22', selesai: 14, pending: 0 },
    ];
    ...
    return {
      ...
      selesai: selesai || 313,
      belum: belum || 16,
      rate: rate > 0 ? rate.toFixed(1) : '95.1',
      meterTua: meterTua || 256,
      meterGangguan: meterGangguan || 73,
      prabayar: prabayar || 258,
      paskabayar: paskabayar || 71,
      estimatedKwh: estimatedKwh || 11840,
      estimatedRp: estimatedRp || 17105248,
      officerList,
      tarifChartData,
      dailyTrendData
    };
  ...
  return (
    <div className="space-y-4">
      ...
      {/* 2. Sub-Tabs Navigation (Clean Bar Style matching Menu 2 & 5) */}
      <div className="bg-white p-1 rounded-xl border border-slate-200 shadow-2xs flex items-center gap-1.5 overflow-x-auto">
        <button
        ...
        >
          ...
          <span>Ringkasan Eksekutif & AI Insight</span>
        </button>
        ...
        >
          ...
          <span>Produktivitas Petugas (17 Tim)</span>
        </button>
        ...
        >
          ...
          <span>Distribusi Tarif & Daya</span>
        </button>
        ...
        >
          ...
          <span>Penyelamatan Susut kWh</span>
        </button>
      </div>

      {/* 4. TAB 1: Ringkasan Eksekutif & AI Insight */}
      {activeTab === 'eksekutif' && (
        <div className="space-y-4">
          ...
          <div className="bg-gradient-to-r from-[#0d233a] via-[#102d4b] to-[#0c1f33] text-white rounded-xl p-5 border border-slate-700/60 shadow-md relative overflow-hidden">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-5">
              ...
              <div className="bg-white/5 border border-white/10 rounded-lg p-3.5 backdrop-blur-xs">
                ...
                <p className="text-2xl font-black text-amber-400 font-mono my-1 tracking-tight">
                  {metrics.belum} Unit
                </p>
                <p className="text-[11px] text-slate-300">
                  Terkonsentrasi pada petugas Yusril (10), Andre (3), dan Gabriel (3).
                </p>
              </div>
            ...
            <div className="bg-black/30 border border-white/10 rounded-lg p-3.5 text-xs text-slate-200 space-y-2">
              ...
              <ul className="space-y-1.5 pl-2 text-[11px] text-slate-300 leading-relaxed">
                <li className="flex items-start gap-1.5">
                  ...
                  <span><strong className="text-white">Re-alokasi Petugas:</strong> Berikan bantuan personil pendamping kepada <em className="text-amber-300 not-italic font-bold">YUSRIL</em> untuk mempercepat penyelesaian sisa 10 pelanggan di wilayah Halong/Waiheru.</span>
                </li>
                <li className="flex items-start gap-1.5">
                  <span className="text-amber-400 shrink-0">•</span>
                  <span><strong className="text-white">Stok Material kWh & MCB:</strong> Pastikan penyediaan stok kWh meter prabayar 1 Phasa 5(60)A minimal 350 unit untuk target peremajaan bulan depan.</span>
                </li>
                <li className="flex items-start gap-1.5">
                  <span className="text-amber-400 shrink-0">•</span>
                  <span><strong className="text-white">Audit Stand Bongkar:</strong> Seluruh stand bongkar pada pelanggan pascabayar (status XP2TL) telah terdokumentasi rapi untuk mencegah sengketa tagihan susulan (TS P2TL).</span>
                </li>
              </ul>
            </div>
          </div>
      ...
      {/* 5. TAB 2: Produktivitas Petugas (17 Tim) */}
      {activeTab === 'petugas' && (
      ...
      {/* 7. TAB 4: Penyelamatan Susut kWh */}
      {activeTab === 'susut' && (
        <div className="space-y-4 bg-white p-5 rounded-xl border border-slate-200 shadow-2xs">
          {/* Header */}
          <div>
